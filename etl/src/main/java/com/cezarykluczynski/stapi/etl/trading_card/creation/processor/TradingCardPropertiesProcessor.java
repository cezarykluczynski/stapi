...
@Service
@Slf4j
public class TradingCardPropertiesProcessor implements ItemProcessor<Element, TradingCardPropertiesProcessor.TradingCardProperties> {
	...
	@Inject
	public TradingCardPropertiesProcessor(TradingCardSetItemsProcessor tradingCardSetItemsProcessor) {
	...
	public TradingCardProperties process(Element item) throws Exception {
		...
		for (String part : parts) {
			...
			if (copyrightMatcher.matches()) {
				...
				tradingCardProperties.setProductionRun(tradingCardSetItemsProcessor.process(numberMadePattern.group(1)));
	...
	@Data
	static class TradingCardProperties {
