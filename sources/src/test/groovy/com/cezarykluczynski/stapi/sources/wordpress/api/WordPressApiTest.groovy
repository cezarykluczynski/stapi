...
class WordPressApiTest extends Specification {
	...
	void "gets all pages from a WordPress API"() {
		...
		when:
		List<Page> pageList = wordPressApi.getAllPages(WordPressSource.STAR_TREK_CARDS)

		then:
		1 * wordPressAfrozaarConnectorMock.getPage(WordPressApi.PAGES_URL_TEMPLATE, 1, WORD_PRESS_SOURCE) >> pagePagedResponse1
		1 * wordPressAfrozaarConnectorMock.getPage(WordPressApi.PAGES_URL_TEMPLATE, 2, WORD_PRESS_SOURCE) >> pagePagedResponse2
		1 * wordPressAfrozaarConnectorMock.getPage(WordPressApi.PAGES_URL_TEMPLATE, 3, WORD_PRESS_SOURCE) >> pagePagedResponse3
