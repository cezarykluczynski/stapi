...
class StapiSoapPortTypesProviderTest extends AbstractStapiClientTest {
	...
	void "provider can be instantiated with canonical URL"() {
		...
		((String) toBindingProvider(stapiSoapPortTypesProvider.astronomicalObjectPortType).requestContext.get(URL_KEY))
				.contains(StapiClient.CANONICAL_API_URL)
		...
		((String) toBindingProvider(stapiSoapPortTypesProvider.comicSeriesPortType).requestContext.get(URL_KEY))
				.contains(StapiClient.CANONICAL_API_URL)
		...
		((String) toBindingProvider(stapiSoapPortTypesProvider.comicStripPortType).requestContext.get(URL_KEY))
				.contains(StapiClient.CANONICAL_API_URL)
		((String) toBindingProvider(stapiSoapPortTypesProvider.comicCollectionPortType).requestContext.get(URL_KEY))
				.contains(StapiClient.CANONICAL_API_URL)
	}
