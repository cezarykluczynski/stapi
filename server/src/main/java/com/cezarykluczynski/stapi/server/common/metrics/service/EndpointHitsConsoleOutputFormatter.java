...
public class EndpointHitsConsoleOutputFormatter {
		...
		entitiesToReportRows.entrySet()
				.forEach(stringReportRowEntry -> {
					...
					if (reportRow.totalHits > 0) {
						...
						double restHitsPercentage = (double) 100 * restHits / reportRow.totalHits;
						double soapHitsPercentage = 100 - restHitsPercentage;
		...
		nonEmptyEntitiesToReportRows.entrySet()
				...
				.forEach(stringReportRowEntry -> {
					...
					if (TOTAL.equals(entityName) && nonEmptyEntitiesToReportRows.size() > 1) {
						stringBuilder.append(String.format("%n%s", StringUtils.leftPad(DASH, line.length(), DASH)));
					}
				});
	...
	private static class ReportRow {
		...
		int soapHits;
		int totalHits;
		Double restHitsPercentage;
		Double soapHitsPercentage;
