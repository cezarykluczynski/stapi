...
class EndpointHitsReaderTest extends Specification {
	...
	private static final long REST_ENDPOINT_V2_NUMBER_OF_HITS = 5
	...
	void "reads hits when refreshed"() {
		...
		EndpointHit restEndpointV2Hit = new EndpointHit(endpointName: 'PerformerV2RestEndpoint', numberOfHits: REST_ENDPOINT_V2_NUMBER_OF_HITS)
		...
		classSimpleNameToClassMap.put('Performer', Performer)
		...
		1 * endpointHitRepositoryMock.findAll() >> Lists.newArrayList(restEndpointHit, soapEndpointHit, restEndpointV2Hit, ignoredEndpointHit)
		...
		endpointHitsReader.readAllHitsCount() == REST_ENDPOINT_NUMBER_OF_HITS + SOAP_ENDPOINT_NUMBER_OF_HITS + REST_ENDPOINT_V2_NUMBER_OF_HITS
		...
		endpointHits.get(Performer) == REST_ENDPOINT_V2_NUMBER_OF_HITS
	}
